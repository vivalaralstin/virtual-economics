...
FROM (
	...
	FROM dbtable2.user_currencies_transactions a
	WHERE amount > 0
	AND currencyid = 1
	AND source != 'cs'
	AND source != 'admin'
	AND userid not in (SELECT distinct userid FROM bp.agg.employee_whitelist)
	AND TO_CHAR(CONVERT_TIMEZONE('UTC','America/Los_Angeles', a.addtime::timestamp_ntz), 'YYYY-MM-DD')  = '%s'
			...
			LEFT OUTER JOIN (
				...
				FROM bp.public.raw_para_json d, LATERAL FLATTEN(input => parse_json(d.src:attacker_level)) f
